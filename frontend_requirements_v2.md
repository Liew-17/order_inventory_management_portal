# 轻量级订货与库存管理门户 - 前端需求与 UI/UX 文档 (V2.0)
...
- **UI 布局**: 顶部为搜索框与**“我的订单”导航按钮**（点击跳转至 `/orders`），下方为商品卡片网格（Grid）。
- **组件 `ProductCard`**: 
  - 显示商品名称、SKU、价格。
...
### 4.4 订单列表页 (Orders List Page) - `/orders`
- **UI 布局**: 顶部包含搜索框（支持输入 Order ID 查询）以及状态筛选标签（如 All, Pending Payment, Under Review, Completed）。下方为订单卡片列表。
- **功能**:
  - 列表展示历史订单，每张卡片需显示：Order ID、总金额 (`Total Amount`)、下单时间，以及醒目的当前订单状态 (`Status`) 标签。
  - 点击整张订单卡片或“查看详情”按钮，即可路由跳转至对应的订单详情页 (`/order/:id`)。
- **数据交互**: 调用 `GET /api/v1/orders` 获取订单列表数据。
...
4. **语言强制要求 (CRITICAL: Language Requirement)**: 所有前端的 UI 文本（包括按钮、表单标签、Toast 提示、占位符等）以及代码中的变量/函数命名，**必须全部使用英文 (English)**。
